 ...
 * ## Why it was written
 *
 * Because the absence of it was measured, on the other branch of this template
 * tree, and every single list there was incomplete: jolly-roger listed 0 of 3,
 * template-commit-reveal 0 of 17 (its file said "this template omits nothing"),
 * reveal-or-die 4 of 52, bleeps 1 of 21, mandalas 2 of 24. On THIS branch the
 * same audit found `ronan-eth` dropping 2 paths and `conquest-website-2` dropping
 * 6, with no list anywhere to record either - which is why the machinery is here,
 * at the nearest ancestor both of them share, rather than copied into each.
 *
 * THIS TEMPLATE'S OWN LIST IS EMPTY, and that is not a reason to move the check
 * down. It is empty because this template drops nothing from `template-svelte-tailwind`
 * (measured, not assumed), and it is HERE because its two descendants do drop
 * things and would otherwise each need their own copy of this file. An ancestor is
 * the distribution point for shared machinery even when it has nothing of its own
 * to guard; what would be wrong is putting a LIST somewhere its entries are not
 * true. The cost is exactly what this file's header predicts:
